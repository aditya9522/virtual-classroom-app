...
from app.models.classroom import Class, ClassCreate
...
async def list_classes_for_user(session: AsyncSession, user: User, skip: int = 0, limit: int = 100) -> List[Class]:
    if user.role == Role.TEACHER:
        result = await session.exec(sql_select(Class).where(Class.teacher_id == user.id).offset(skip).limit(limit))
    elif user.role == Role.STUDENT:
        enrolled_class_ids = await get_enrolled_class_ids(session, user.id)
        result = await session.exec(sql_select(Class).where(Class.id.in_(enrolled_class_ids)).offset(skip).limit(limit))
    else:  # ADMIN
